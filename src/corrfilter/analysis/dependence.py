...
quantities with the SAME estimator-selection rule as scripts/dependence/compute_correlation.py (listwise
Ledoit--Wolf when abstention is low, pairwise-complete + shrinkage otherwise), so
outputs match the published tables exactly.

CLI:
  python corrfilter.analysis.dependence --bank configs/judge_bank.yaml \
...
def estimate_R(V, M, gold, listwise_floor=LISTWISE_FLOOR):
    """Return (R, estimator_name, min_joint, listwise_retention) matching scripts/dependence/compute_correlation.py."""
