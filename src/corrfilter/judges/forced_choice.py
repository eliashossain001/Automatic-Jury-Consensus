...
class ForcedChoiceJudge(HFJudge):
    ...
    def __init__(self, *args, position_override: dict[str, bool] | None = None, **kwargs):
        ...
        The inherited ``_position_swap`` derives the slot from ``hash()``, which Python
        randomises per process unless PYTHONHASHSEED is fixed. Position assignments are
        therefore NOT reproducible across runs: the published base and GRPO caches agree
        on slot for only 51.4% of items, i.e. chance. Since position bias is this bank's
        strongest measured vulnerability (slot gaps up to 70 points), comparing banks that
        saw different slot layouts confounds the comparison. Passing the published base
        bank's own slot bits here makes every bank see identical items in identical slots
        and makes the forced-choice-vs-cached validation like-for-like.
        """
