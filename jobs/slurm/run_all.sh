...
# Usage:   bash jobs/slurm/run_all.sh [--cpu-only] [--no-submit]
# Override anything via env, e.g.:  SLURM_GRES=gpu:nvidia_h100_80gb_hbm3:1 bash jobs/slurm/run_all.sh
# =============================================================================
set -euo pipefail
...
# IMPORTANT: these import pandas/sklearn/matplotlib and OOM on the the cluster login node's
...
if [ "$LOCAL" = 1 ] || ! command -v sbatch >/dev/null; then
  ...
  J_CPU=$(SB jobs/slurm/cpu_stages.slurm)
...
if command -v sbatch >/dev/null && [ "$NO_SUBMIT" = 0 ]; then
  ...
  J_DPOTR=$(SB --gres="$SLURM_GRES" jobs/slurm/train_dpo_judges.slurm)
  J_DPOEV=$(SB --gres="$SLURM_GRES" --dependency=afterok:"$J_DPOTR" jobs/slurm/eval_dpo_judges.slurm)
  # Bucket 3: downstream policies (independent) -> summary
  J_PMAJ=$(SB --gres="$SLURM_GRES" jobs/slurm/train_policy_majority.slurm)
  J_PCF=$(SB  --gres="$SLURM_GRES" jobs/slurm/train_policy_corrfilter.slurm)
  J_SUM=$(SB  --dependency=afterok:"$J_PMAJ":"$J_PCF" jobs/slurm/downstream_summary.slurm)
  # Bucket 4: extended bank (independent)
  J_EXT=$(SB --gres="$SLURM_GRES" jobs/slurm/eval_extended_bank.slurm)
  ...
  echo "[run_all] no sbatch (or --no-submit): GPU stages NOT launched. Run each jobs/slurm/*.slurm manually."
  SUBMITTED="none"
fi
fi
