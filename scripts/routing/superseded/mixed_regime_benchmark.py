...
Every instance is evaluated with the `scripts/routing/regime_router.py` and `scripts/routing/unified_router.py` protocol: 100-item labeled
...
globally per mixture), router_hard (scripts/routing/regime_router.py clean-referenced rule),
router_soft (scripts/routing/unified_router.py softmax blend), oracle_router, oracle_filter.
...
Usage: python scripts/routing/superseded/mixed_regime_benchmark.py --project-root .
...
from corrfilter.cfi.consensus import majority_consensus, supermajority_consensus
from corrfilter.correlation import effective_size
...
def build_instances(root: Path):
    ...
    # clean-bank reference (scripts/routing/regime_router.py)
...
def main() -> None:
    ...
    with open(out_dir / "summary.md", "w") as f:
        ...
        f.write(f"{len(instances)} synthetic instances (18 per regime; regime label = generator, "
                "by construction). Protocol identical to `scripts/routing/regime_router.py` and `scripts/routing/unified_router.py`: small_gold R from a "
