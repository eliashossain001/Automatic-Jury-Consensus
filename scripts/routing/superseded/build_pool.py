...
Research code only; the paper and the scripts/routing/superseded/mixed_regime_benchmark.py benchmark are untouched. The
...
Each instance is evaluated with the scripts/routing/superseded/mixed_regime_benchmark.py protocol (same filters, matched
...
Usage: python scripts/routing/superseded/build_pool.py
...
spec37 = importlib.util.spec_from_file_location("mrb", ROOT / "scripts" / "mixed_regime_benchmark.py")
mrb = importlib.util.module_from_spec(spec37)
spec37.loader.exec_module(mrb)

BASE = 20260801           # distinct from scripts/routing/superseded/mixed_regime_benchmark.py's instance seeds
...
def main() -> None:
    ...
    pd.DataFrame(feat_rows).to_csv(out / "pool_features.csv", index=False)
    pd.DataFrame(method_rows).to_csv(out / "pool_methods.csv", index=False)
