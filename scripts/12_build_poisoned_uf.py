...
judge run (scripts/13) votes on the content-canonical pair and is independent of
the label, so a single vote cache serves all corruption rates; corruption is
applied at evaluation time (scripts/14).

Output: outputs/synthetic_poisoned_ultrafeedback/poisoned_manifest.csv
        (+ poisoned_manifest_summary.json)

Usage:
    python scripts/12_build_poisoned_uf.py --n 2000 --project-root .
"""
