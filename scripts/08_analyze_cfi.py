...
Reads the cached biased votes (scripts/07) + clean H1 votes + H1 clean R, then
...
Usage:
    python scripts/08_analyze_cfi.py --config configs/cfi_bias_prompts.yaml \
...
def main() -> None:
    ...
    if not available_mechs:
        msg = ("No cached biased votes found under "
               f"{votes_root}. Run scripts/07_run_cfi_bank.py first.")
