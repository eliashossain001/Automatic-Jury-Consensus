...
Stages (deterministic item manifests, disjoint by construction):
  ...
  pilot  ~150 items  judge-competence gate (scripts/crosstask/factuality_pilot_report.py reports it)
  main   ~1000 items clean vote bank (includes pilot items: same clean
                     distribution, votes reused from cache when present)

Usage:
  python scripts/crosstask/run_factuality_judges.py --stage smoke --live
  python scripts/crosstask/run_factuality_judges.py --stage pilot --live
  python scripts/crosstask/run_factuality_judges.py --stage main  --live
