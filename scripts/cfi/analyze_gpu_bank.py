...
Reads the cached biased votes (scripts/cfi/run_bank_gpu.py) + clean H1 votes + H1 clean R, then
...
Usage:
    python scripts/cfi/analyze_gpu_bank.py --config configs/cfi_bias_prompts.yaml \
...
def main() -> None:
    ...
    if not available_mechs:
        msg = ("No cached biased votes found under "
               f"{votes_root}. Run scripts/cfi/run_bank_gpu.py first.")
