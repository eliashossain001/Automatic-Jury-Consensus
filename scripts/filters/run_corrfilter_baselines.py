...
(requires cached biased votes from scripts/regimes/run_cofailure_votes_gpu.py) — the setting where correlated
    ...
    if args.mechanism:
        ...
        if sum(len(v) for v in biased_views.values()) == 0:
            raise FileNotFoundError(
                f"no cached biased votes for '{args.mechanism}'; run scripts/regimes/run_cofailure_votes_gpu.py first")
