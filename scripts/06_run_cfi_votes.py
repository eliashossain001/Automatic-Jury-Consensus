...
Usage:
    python scripts/06_run_cfi_votes.py [--config configs/cfi.yaml] [--mode replay|real]
"""
...
def main() -> None:
    ...
    if not manifest_path.exists():
        raise FileNotFoundError(
            f"CFI manifest not found at {manifest_path}; run scripts/05_build_cfi_manifest.py first"
        )
