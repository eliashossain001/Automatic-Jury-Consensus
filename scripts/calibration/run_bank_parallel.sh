...
#
# Usage:
#   bash scripts/calibration/run_bank_parallel.sh          # full calibration set
#   bash scripts/calibration/run_bank_parallel.sh 200      # smoke (first 200 items)
#
...
CUDA_VISIBLE_DEVICES=0 python3 scripts/calibration/run_judge_bank.py \
  $(build_args "${GPU0_JUDGES[@]}") ${LIMIT_ARG} \
  > "${GPU0_LOG}" 2>&1 &
PID0=$!

CUDA_VISIBLE_DEVICES=1 python3 scripts/calibration/run_judge_bank.py \
