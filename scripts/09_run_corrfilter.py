...
(requires cached biased votes from scripts/07) — the setting where correlated
...
Usage:
    python scripts/09_run_corrfilter.py --project-root /home/elias/elias_projects/corrfilter
    python scripts/09_run_corrfilter.py --mechanism position_bias_stress_test --target-retention 0.8
"""
    ...
    if args.mechanism:
        ...
        if sum(len(v) for v in biased_views.values()) == 0:
            raise FileNotFoundError(
                f"no cached biased votes for '{args.mechanism}'; run scripts/07 first")
