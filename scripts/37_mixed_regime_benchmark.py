...
Every instance is evaluated with the scripts/18-19 protocol: 100-item labeled
...
globally per mixture), router_hard (scripts/18 clean-referenced rule),
router_soft (scripts/19 softmax blend), oracle_router, oracle_filter.
...
Usage: python scripts/37_mixed_regime_benchmark.py --project-root .
...
def build_instances(root: Path):
    ...
    # clean-bank reference (scripts/18)
...
def main() -> None:
    ...
    with open(out_dir / "summary.md", "w") as f:
        ...
        f.write(f"{len(instances)} synthetic instances (18 per regime; regime label = generator, "
                "by construction). Protocol identical to scripts/18-19: small_gold R from a "
