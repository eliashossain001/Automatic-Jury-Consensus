...
serves all corruption rates (corruption is applied at evaluation time, scripts/regimes/evaluate_poisoned_ultrafeedback.py).

Efficiency mirrors scripts/regimes/run_cofailure_votes_gpu.py: load each of the five base models once and sweep
...
Usage:
    ...
    python scripts/regimes/run_ultrafeedback_judges.py --project-root .
    # smoke: python scripts/regimes/run_ultrafeedback_judges.py --base-id phi-3.5-mini --max-items 24
"""
...
logger = logging.getLogger("scripts.regimes.run_ultrafeedback_judges")
