...
#
# Usage:
#   bash scripts/run_cfi_experiment.sh
#   bash scripts/run_cfi_experiment.sh --mode real        # GPU re-inference
#
...
python scripts/05_build_cfi_manifest.py --config "${CONFIG}" \
  > "${LOG_DIR}/script05.log" 2>&1

echo "[2/4] running CFI votes (mode=${MODE})"
python scripts/06_run_cfi_votes.py --config "${CONFIG}" --mode "${MODE}" \
  > "${LOG_DIR}/script06.log" 2>&1

echo "[3/4] analyzing CFI"
python scripts/07_analyze_cfi.py --config "${CONFIG}" \
  > "${LOG_DIR}/script07.log" 2>&1

echo "[4/4] plotting CFI"
python scripts/08_plot_cfi.py --config "${CONFIG}" \
