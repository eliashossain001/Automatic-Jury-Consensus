...
judge run (scripts/attacks/run_poisoned_uf_judges.py) votes on the content-canonical pair and is independent of
the label, so a single vote cache serves all corruption rates; corruption is
applied at evaluation time (scripts/attacks/eval_poisoned_uf.py).
