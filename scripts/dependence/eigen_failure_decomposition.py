...
and the ``correlation.npz`` produced by ``scripts/dependence/compute_correlation.py``
...
from corrfilter.analysis import top_eigen_directions
from corrfilter.analysis.eigen_failure import (
    ALIGNMENT_FAMILIES,
    ALL_TAGS,
    alignment_scores,
...
from corrfilter.correlation import compute_error_matrix
...
def _build_swap_matrix(
    ...
    for i, lid in enumerate(logical_ids):
        ...
        for row in df.itertuples(index=False):
            ...
            if j is not None:
                swap[j, i] = int(bool(row.position_swapped))
    return swap
