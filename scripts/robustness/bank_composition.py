...
Usage: python scripts/robustness/bank_composition.py --project-root .
"""
...
from corrfilter.analysis.h1_test import pairwise_contrast_bootstrap
...
def _load_vs(cache, lids, ids):
    ...
    for j, lid in enumerate(lids):
        ...
        for r in dfj.itertuples(index=False):
            ...
            if v == ABSTAIN:
                continue
            V[i, j] = v; M[i, j] = 1; S[i, j] = 1 if bool(r.position_swapped) else 0
    return V, M, S
