...
#
# Usage:
#   bash scripts/pipelines/run_cofailure_experiment.sh
#   bash scripts/pipelines/run_cofailure_experiment.sh --mode real        # GPU re-inference
#
...
python scripts/regimes/build_cofailure_manifest.py --config "${CONFIG}" \
  > "${LOG_DIR}/script05.log" 2>&1

echo "[2/4] running CFI votes (mode=${MODE})"
python scripts/regimes/run_cofailure_votes_replay.py --config "${CONFIG}" --mode "${MODE}" \
  > "${LOG_DIR}/script06.log" 2>&1

echo "[3/4] analyzing CFI"
python scripts/regimes/compute_cofailure_metrics.py --config "${CONFIG}" \
  > "${LOG_DIR}/script07.log" 2>&1

echo "[4/4] plotting CFI"
python scripts/regimes/plot_cofailure_figures.py --config "${CONFIG}" \
