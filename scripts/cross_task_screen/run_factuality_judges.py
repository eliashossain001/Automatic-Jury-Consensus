...
Stages (deterministic item manifests, disjoint by construction):
  ...
  pilot  ~150 items  judge-competence gate (scripts/cross_task_screen/report_factuality_pilot.py reports it)
  main   ~1000 items clean vote bank (includes pilot items: same clean
                     distribution, votes reused from cache when present)

Usage:
  python scripts/cross_task_screen/run_factuality_judges.py --stage smoke --live
  python scripts/cross_task_screen/run_factuality_judges.py --stage pilot --live
  python scripts/cross_task_screen/run_factuality_judges.py --stage main  --live
